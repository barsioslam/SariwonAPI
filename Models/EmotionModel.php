...
class EmotionModel extends Model {
    ...
    public function findById(int $id): ?array {
        $this->query('SELECT * FROM `emotion` WHERE `id` = :id LIMIT 1', ['id' => $id]);
    ...
    public function create(string $name, string $description = null, int $serverId = null, int $createdBy = null): int {
        $this->query(
            'INSERT INTO `emotion` (`name`, `description`, `server_id`, `created_by`)
    ...
    public function update(int $id, array $data): bool {
        ...
        return $this->query("UPDATE `emotion` SET $sets WHERE `id` = :id", $fields);
    }

    public function delete(int $id): bool {
        return $this->query('DELETE FROM `emotion` WHERE `id` = :id', ['id' => $id]);
    }
